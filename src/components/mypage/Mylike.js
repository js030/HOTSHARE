...
export default function MyLike() {
    ...
    const likesHotelsQuery = useQuery({
        ...
        queryFn: () =>
            instance
                .get(`api/v1/members/me/likes?page=${page - 1}`, {
                    ...axios.defaults,
                    useAuth: true,
                })
                .then((res) =>
    ...
    const pages = useMemo(
        () => likesHotelsQuery.data?.objData.totalPages ?? 0,
        [likesHotelsQuery.data?.objData.totalPages]
    );
    ...
    return (
        <div className="flex h-screen">
            <div className={"flex flex-col w-full gap-5 px-5"}>
                ...
                {likesHotelsQuery.isLoading &&
                    Array(4)
                        .fill(0)
                        .map((_, index) => {
                            return <Skeleton key={index} height={200}/>;
                        })}
                {likesHotelsQuery.isSuccess &&
                    likesHotelsQuery.data?.objData.content.map((hotel, index) => {
                        return (
                            <Card key={index} className={"!overflow-visible"}>
                                <CardBody>
                                    <div className={"flex gap-5"}>
                                        <Image src={hotel.imagesResponse.imageUrl[0]} width={150}/>
                                        ...
                                        <div className={"flex flex-col ml-auto justify-between"}>
                                            <Button onClick={() => cancelLike.mutate(hotel.id)}>
                                                찜 취소
                                            </Button>

                                            <Button as={Link} href={`/hotel/${hotel.id}`}>
                                                상세보기
                                            </Button>
                                        </div>
                                    </div>
